...
def get_IA_terms(
    ...
    # before: Get nonlinear Pk. lin Pk is already read-in at the fast-pt k grid from the fastpt block).
    # Read the split power spectrum instead of the fast-pt power spectrum
    P_IA_dict["P_nl"] = P_nl
    P_IA_dict["Plin"] = P_lin
...
def execute(block, config):
    ...
    omega_m = block[cosmo, "omega_m_growth"]

    # Load the matter power spectra
    z_lin, k_lin, p_lin = block.get_grid(lin, "z", "k_h", "P_K")
    z_nl, k_nl, p_nl = block.get_grid(nl, "z", "k_h", "P_K")

    # take growth factor from the ODE D(z)=G(z)/(1+z)
    # as in the standard (non-split) implementation this is normalised to D(0)=1
    Dz = (block[growth, "G_z_growth"] / (1 + z_nl)) / (
        block[growth, "G_z_growth"][0] / (1 + z_nl[0]))
