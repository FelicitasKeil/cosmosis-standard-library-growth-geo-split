...
def cl_to_xi_to_block_eb(block, output_section, name,
    ...
    for (cl_interp, o, leg) in zip(cl, output_section, legfacs ):
        ...
        # if name == "bin_4_4" and o == "shear_xi_plus":
        #     with open("thetas.pkl", "wb") as out_pkl:
        #         pickle.dump(radians_to_arcmin(thetas), out_pkl)

        #     with open("xi_plus_om-geo-03-growth-035-early-03_prefactor.pkl", "wb") as out_pkl:
        #         pickle.dump(xis, out_pkl)

        # if name == "bin_4_4" and o == "shear_xi_minus":
        #     with open("xi_minus_om-geo-03-growth-035-early-03_prefactor.pkl", "wb") as out_pkl:
        #         pickle.dump(xis, out_pkl)
...
def cl_to_xi_to_block(block, output_section, name,
    ...
    for (o,leg) in zip( output_section, legfacs ):
        ...
        for it, t in enumerate(thetas):
            xis[it] = np.sum(cl_interp * leg[it])
        block[o, name] = xis
        # if name == "bin_1_1" and o == "galaxy_xi":
        #     with open("w_theta_om-geo-03-growth-035-early-03_prefactor.pkl", "wb") as out_pkl:
        #         pickle.dump(xis, out_pkl)

        # if name == "bin_1_4" and o == "galaxy_shear_xi":
        #     with open("gamma_t_om-geo-03-growth-035-early-03_prefactor.pkl", "wb") as out_pkl:
        #         pickle.dump(xis, out_pkl)
...
def cl_to_xi_plus_and_minus_precomp(cl_input, thetas, G_plus_minus_pre):
    """Compute xi+/- given:
    cl_input: Interpolation function for cl
