...
static double* name;  // Pointer version
...
{
    if (name == NULL) {
        name = malloc(sizeof(double));  // Allocate memory
    }
    *name = val;
    return 0;
}
	...
	parname nuisance_names[n_nuisance], double *p, double *cl)
{
    ...
    printf("starting test functions \n");
    double value = 3.14;
    put_double(value);
    double result;
    get_double(&result);
    printf("result of test function %f \n", result);

	// ... and now get the nuisance parameters
	double val;
	c_datablock_get_double(block, PLANCK_SECTION, "cib_index", &val);
	printf("Test to print cib_index: %f \n", val);
	double val_2;
	c_datablock_get_double(block, PLANCK_SECTION, "A_sbpx_100_100_TT", &val_2);
	printf("Test to print A_sbpx_100_100_TT: %f \n", val_2);

	double A_sbpx_100_100_TT = 1.0;
	int put_status = c_datablock_replace_double(block, PLANCK_SECTION, "A_sbpx_100_100_TT", A_sbpx_100_100_TT);
	printf("Replace status: %d\n", put_status);

	double val_3 = 0.0;
	int get_status = c_datablock_get_double(block, PLANCK_SECTION, "A_sbpx_100_100_TT", &val_3);
	printf("Get status: %d\n", get_status);
	printf("Test to put double A_sbpx_100_100_TT: %f \n", val_3);

	printf("nuisance parameters needed \n");
